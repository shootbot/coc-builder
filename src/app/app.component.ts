...
import { BuildingSpec, SPECS, INITIAL_INVENTORY, InventoryState, PlacedBuilding, BoardPlaceEvent, BoardMoveEvent } from './types';
...
export class AppComponent {
  ...
  specs: Record<string, BuildingSpec> = SPECS;
  inventory = signal<InventoryState>(structuredClone(INITIAL_INVENTORY));
  ...
  reset() {
    this.inventory.set(structuredClone(INITIAL_INVENTORY));
    this.placed.set([]);
    this.dragGhost.set(null);
  }
    ...
    input.onchange = () => {
      ...
      f.text().then(t => {
        ...
        if (Array.isArray(data.placed)) {
          ...
          const inv = structuredClone(INITIAL_INVENTORY);
          for (const b of data.placed) { if (inv[b.key]) inv[b.key].count = Math.max(0, inv[b.key].count - 1); }
          this.inventory.set(inv);
